...
// alone.
//
// ОТЛИЧИЕ ОТ ОРИГИНАЛА (единственное во всём engine): `paint`/`paintLines`
// больше не рисуют в `CanvasRenderingContext2D`, а складывают уже разрешённые
// примитивы в `OrbSink`. Вся математика ниже — дословно из thinking-orbs;
// сами режимы (orbits, lattice, web, …) не тронуты вообще.
...
/** Кадр после разрешения темы: серый уже посчитан, рисовать можно вслепую. */
export interface OrbSink {
  /** [x, y, r, gray 0..1, alpha] на точку. */
  dots: number[];
  /** [x1, y1, x2, y2, gray 0..1, alpha, width] на линию. */
  lines: number[];
}

export const createSink = (): OrbSink => ({ dots: [], lines: [] });
 ...
 */
export function paint(sink: OrbSink, dots: Dot[], dark: boolean, rMin = 0.3): void {
  dots.sort((a, b) => a.z - b.z);
  for (const d of dots) {
    const alpha = d.a ?? 1;
    if (alpha < 0.02) continue;
    const w = Math.min(1, Math.max(0, d.white));
    sink.dots.push(d.x, d.y, Math.max(rMin, d.r), dark ? 1 - w : w, alpha);
  }
}

/** Stroke pass for edge-based modes. Runs before `paint` so nodes sit on top. */
export function paintLines(sink: OrbSink, lines: Line[], dark: boolean): void {
  for (const l of lines) {
    const alpha = l.a ?? 1;
    if (alpha < 0.02) continue;
    const w = Math.min(1, Math.max(0, l.white));
    sink.lines.push(l.x1, l.y1, l.x2, l.y2, dark ? 1 - w : w, alpha, l.w);
  }
}
